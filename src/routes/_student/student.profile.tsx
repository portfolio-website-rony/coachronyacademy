...
function ProfilePage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ display_name: name, phone, bio })
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <form onSubmit={save} className="glass space-y-3 rounded-2xl p-6">
        ...
        <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Short bio" maxLength={500} rows={3} className="glass w-full rounded-xl px-4 py-3 text-sm outline-none" />
